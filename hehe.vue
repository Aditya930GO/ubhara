<template>
  <div class="rd-layout" ref="rdLayout">
    <rd-alert />
    <header
      v-if="user && viewMode !== 'desktop' && route.path !== '/login'"
      class="rd-header"
      ref="rdHeader"
    >
      <button
        class="rd-header-navigation-button"
        @click="navigationHandler(navigationOpened ? 'close' : 'open')"
      >
    ...
    <section
      ...
      ref="rdOverview"
    >
      <rd-mini-profile style="margin-bottom: 1rem" @logout="exit" />
      <rd-panel-transactions />
    </section>

    <rd-transaction-cancel-panel
      v-if="panelOpened === 'transaction-cancel'"
      :state="panelState"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-form-panel
      v-if="panelOpened === 'transaction-form'"
      :state="panelState"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-details-panel
      v-if="panelOpened === 'transaction-details'"
      :state="'idle'"
      :data="panelData[0]"
      @open-panel="panelHandler"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-deliver-form-panel
      v-if="panelOpened === 'transaction-deliver-form'"
      :state="'idle'"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-payment-panel
      v-if="panelOpened === 'transaction-payment'"
      :state="'idle'"
      :data="panelData[0]"
      @open-panel="panelHandler"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-payment-form-panel
      v-if="panelOpened === 'transaction-payment-form'"
      :state="'idle'"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-payment-fulfill-panel
      v-if="panelOpened === 'transaction-payment-fulfill'"
      :state="'idle'"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
    <rd-transaction-payment-warn-panel
      v-if="panelOpened === 'transaction-payment-warn'"
      :state="'idle'"
      :data="panelData[0]"
      @exit="panelHandler({ state: 'hide' })"
    />
