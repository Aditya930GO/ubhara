...
export default () => {
  ...
  const getCustomersNames = async (): Promise<CustomerMin[]> => {
    try {
      const response: Response = await $fetch(`${config.public.apiBase}/customers/names`, 'get')
      if (response.status !== 200) throw new Error('')
  ...
  const getCustomerOverview = async (): Promise<Customer[]> => {
    try {
      const response: Response = await $fetch(`${config.public.apiBase}/customers/overview`, 'get')
      if (response.status !== 200) throw new Error('')
  ...
  const addCustomer = async (payload: CustomerRequest): Promise<string> => {
    try {
      const response: Response = await $fetch(`${config.public.apiBase}/customers`, 'post', JSON.stringify({
        name: payload.name,
  ...
  return { customers, names, getCustomers, getCustomersNames, getCustomerOverview, addCustomer }
}